...
2.  **Navegue até o diretório do projeto**:
    ```bash
    cd /home/ubuntu/blazedemo_jmeter
    ```
3.  **Execute o teste de carga**:
    ```bash
    ./run_load_test.sh
    ```
    Este comando executará o teste de carga, gerará um arquivo `.jtl` com os resultados brutos e um relatório HTML completo no diretório `report/load_test_report`.

4.  **Execute o teste de pico**:
    ```bash
    ./run_peak_test.sh
    ```
    Este comando executará o teste de pico, gerará um arquivo `.jtl` com os resultados brutos e um relatório HTML completo no diretório `report/peak_test_report`.
...
### Análise Esperada
Após a execução dos testes, o relatório HTML gerado pelo JMeter (localizado em `report/load_test_report` e `report/peak_test_report`) fornecerá as seguintes métricas chave:
